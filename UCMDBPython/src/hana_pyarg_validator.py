...
def namedtuple_validator(*type_, **kwarg_to_type):
    def decorator(klass):
        class wrapper_klass(klass):
            __new__ = validate(*type_, **kwarg_to_type)(klass.__new__)
...
def validate(*type_, **kwarg_to_type):
    def decorator_fn(real_fn):
        def wrapper(*args, **kwargs):
            ...
            start_index = 0
            if _has_self(real_fn) or _is__new__(real_fn):
