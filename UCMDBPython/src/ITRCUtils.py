...
def getHostNamesFromShell(ip_string, shell = None, dnsServers = []):    
    resolvers = []
    try:        
...
def getIPsFromShell(hostname, shell = None, dnsServers = []):
...
def getHostNames(ip_string, Framework = None, dnsServers = []):
    localShell = None
    hostname = None
    aliasList = []
...
def getIPs(hostname, Framework = None, dnsServers = []):
    if netutils.isValidIp(hostname):
        return [hostname]
...
def getResourceType(sourceType):
    classModel = ConfigFilesManagerImpl.getInstance().getCmdbClassModel()
    
    if classModel.isTypeOf('node', sourceType):
        return 'devices'
    if classModel.isTypeOf('business_application', sourceType):
        return 'apps'
    #if classModel.isTypeOf('ip_')
...
def getITRCID(obj):
    if obj:
        if getObjKey(obj, 'type') and getObjKey(obj, 'id'):
            return "%s: %s" % (getObjKey(obj, 'type'), getObjKey(obj, 'id'))
        else:
            return None
    return None
...
def isOSHLink(osh):
    ret = False
    if osh and isinstance(osh, ObjectStateHolder):
        return osh.getCmdbIdType() == 13 or osh.getObjectClass() == 'containment'     
    return ret

def setITRCID(obj, osh):
     
    if obj and osh and isinstance(osh, ObjectStateHolder) and not isOSHLink(osh):
        logger.info("building %s OSH for %s" % (osh.getObjectClass(), getITRCID(obj)))
        if getObjKey(obj, 'type') and getObjKey(obj, 'id'):                  
            osh.setStringAttribute("itrc_id", str(getObjKey(obj, 'id')))
            osh.setStringAttribute("itrc_url", getObjKey(obj, 'links.self'))      
            osh.setDateAttribute('itrc_updated', Date())
        
            try:
                osh.setAttribute('itrc_data', json.dumps(getObjKey(obj, 'attributes')))
            except:
                pass       
...
def createIPEndpointOSHV(framework, ipAddress, portNum, portName, hostname = None, protocol = modeling.SERVICEADDRESS_TYPE_TCP):
    ...
    #rsOSH = modeling.createApplicationOSH('web_server', None, hostOSH)
    #rsOSH.setStringAttribute('application_ip', ipAddress)
    #rsOSH.setAttribute('application_ip_domain', ipOSH.getAttributeValue('ip_domain'))
    #rsOSH.setIntegerAttribute('application_port', portNum)
    
    #OSHVResult.add(rsOSH)
    #OSHVResult.add(modeling.createLinkOSH('usage', rsOSH, ipPort))
    #OSHVResult.add(modeling.createLinkOSH('usage', rsOSH, ipOSH))
...
def getIPOSHV(Framework, ip, netmask = None, dnsServers=None):
    OSHVResult = ObjectStateHolderVector()  
    if not ip:
        return OSHVResult
    ...
    try:
        ...
        for ipResult in ipsResult:
            ...
            if alias:
                for alias1 in alias:
                    sv.add(alias1)
            pingedIpOSH.addAttributeToList('itrc_alias', sv)
            ...
            if isVirtual:
                ...
                if alias:
                    for alias1 in alias:
                        sv.add(alias1)
                replyIpOSH.addAttributeToList('itrc_alias', sv)
